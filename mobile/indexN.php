...
<div data-role="page" id="homePage" data-theme="d">
	...
	<?PHP
		...
		if($page=="") {
			...
			echo'<div id="topNewsDiv">';
				...
				for($i=0;$i<10;$i++) {
					...
					echo'<div class="topNewsDiv">
							...
							<div class="topNewsContent"><h2><a href="?lan='.$lan.'&amp;page=news&amp;id='.$r_id.'">'.$r_title.'</a></h2>
    		...
    		<div class="liVideoDescDiv"><a  class="liVideoA" href="https://www.youtube.com/watch?v=BHwb_aGfEs8&list=PLffuHBHoz5dp8TEZG6p0rQ_yoD7ViggSP&index=1" >Արցախյան ուխտագնացություն</a></div>
    		</div>
   		 </li>
			...
			$count = @mysql_num_rows($resh);
			 ...
			 for($i=0;$i<$count;$i++){
				...
				echo'<li>
					...
					<a href="?lan='.$lan.'&amp;page=news&amp;id='.$r_id_r.'" class="titleNews';
		...
		elseif($page=="news" && $id) {
			...
			$row = mysql_fetch_array($resh);
			 ...
			 echo'<h2 class="titleNews">'.$r_title_r.'</h2>';
	...
	<div data-role="footer" id="footerDiv" data-position="fixed">
		...
		<div data-role="navbar">
			...
			<ul>
				...
				<li><a rel='external' href="/mobile/?lan=<?=$lan?>" <?=($page==""?" class='activeAFooter' ":"")?>?lan=<?=$lan?>><?=$main?></a></li>
				...
				<li><a rel='external' href="/mobile/?page=news&amp;lan=<?=$lan?>" <?=($page=="news"?" class='activeAFooter' ":"")?>><?=$_LANG[10]["$lan"]?></a></li>
				...
				<li><a rel='external' href="/mobile/?page=stream&amp;lan=<?=$lan?>" <?=($page=="stream"?" class='activeAFooter' ":"")?>><?=$_LANG[1][$lan]?></a></li>
