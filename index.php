...
if(isset($_GET['act']) && $_GET['act']=="pages" || $_GET['act']=="news" ||$_GET['act']=="archive_programs"||$_GET['act']=="archive_news"|| $_GET['act']=="cat" || $_GET['act']=="ether" || $_GET['act']=="online" || $_GET['act']=="yerkir" || $_GET['act']=="films" || $_GET['act']=="search" || $_GET['act']=="polls" || $_GET['act']=="spec" || $_GET['act']=="prog") $act=$_GET['act']; else $act='';
...
if(isset($_GET['p'])) $p=intval($_GET['p']); if(!$p) $p = 1;
...
elseif($act=="news" && $id){
		...
		if($lan=="hy") $ym_title = $r_title_r." | Նորություններ"; 



		elseif($lan=="en") $ym_title = $r_title_r." | News";
...
<meta name="description" content="<?=str_replace('"',"&quot;",$r_title_r);//$r_title_r?>" />
...
<link rel="image_src" href="/userfiles/<?=$r_pic_r?>"/>



<meta property="og:description" content="<?=str_replace('"',"&quot;",$ym_title1)?>" /> 



<!--<link rel="canonical" href="http://www.yerkirmedia.am/?id=<?=$ip?>"/>-->
...
<div id="menu">
	...
	<? if($lan=="hy") {?>
	   ...
	   <li  style="float:right; padding-right:7px;">
		...
		<span class='lang' style='float:left;  padding:0 8px 0 8px; background:none' ><a href="?lan=hy" <? if($lan=="hy") echo" class='active'"; ?>>Հայ</a></span>

		<span class='lang' style='float:left;  padding:0 8px 0 8px;'><a href="?lan=ru" <? if($lan=="ru") echo" class='active'"; ?>>Рус</a></span>

		<span class='lang' style='float:left;  padding:0 8px 0 8px;'> <a href="?lan=en" <? if($lan=="en") echo" class='active'"; ?>>Eng</a></span></li>
...
-->		
	...
	if($lan=="hy"){
		...
		<div class="header">
			<h2><? if($lan=="hy")echo "Տեղեկատվական այլ կայքեր";
					elseif($lan=="ru") echo"Другие информационные сайты"; 
					else echo"Other news sites";?>
            </h2>
		</div>
		...
		<?
			...
			$resh_banners_14 = mysql_query("SELECT * FROM `banners` WHERE `b_type`=4 AND `id`!=82 ORDER BY id");
		...
		?>
			...
			<a href='?act=films'><img src="userfiles/<?=$f_nkar_b?>" alt="Ցուցադրվելիք ֆիլմերը" style='border:0px;width:276px;' width='276'/></a>
		...
		<div id="curency">
			...
			<!-- <ul>
				...
				<li><b>USD</b> <i><?=$_SESSION['USD']?></i> <img src="s/i/cur_down.png" alt="" /></li>



				<li class="even"><b>EUR</b> <i><?=$_SESSION['EUR']?></i></li>



				<li><b>RUR</b> <i><?=$_SESSION['RUR']?></i></li>
		...
		<?if($lan=="hy") {?>
		<?php /*?><div id="BizNet" style='margin:6px 0px;height:94px;overflow:hidden;'>
		...
		echo"<div id=\"supertop\">";
			...
			$count_h = @mysql_num_rows($resh_h);
				...
				?>
					...
					<?if($h_nkar_b){?><a href="?act=spec&id=<?=$h_id?>"><img src="userfiles/<?=$h_nkar_b?>" alt="" style='border:0px;'/></a><?}?>
		 ...
		 echo"</div>";
			...
			$resh_h = mysql_query("SELECT * FROM `ht_videos` WHERE `b_id` = '".$id."' Order by `id` desc ");
	...
	elseif($act=="films"){
		...
		elseif($act=="cat" && $id){
				...
				for($i=0;$i<$count;$i++){
					...
					if($c_id==$id) echo "style='text-decoration:underline;' ";
		 ...
		 $resh = mysql_query("SELECT * FROM `records` WHERE `r_category` = '".$id."' AND `r_show_$lan` = 1 Order by `r_ord` desc ");
...
<script type="text/javascript">
  ...
  _gaq.push(['_setAccount', 'UA-22969388-1']);
