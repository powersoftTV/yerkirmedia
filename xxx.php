...
.bottom_ban li{
    ...
    echo "<ul>";
     $resh_banners = mysql_query( "SELECT * FROM `banners` WHERE `b_type` = '5' ORDER BY id desc" ) ;
	   ...
	   $b_pic_1 = mysql_result( $resh_banners, $i, "b_pic" ) ;
   	   $b_link_1 = mysql_result( $resh_banners, $i, "b_link" ) ;
       echo "<li><a target='_blank' href=".$b_link_1." ><img alt='' src='userfiles/". $b_pic_1."'></a></li>";			
	}
    echo "</ul>";
    ?>
    </div>
...
function auto_loop(){
		...
		$('.bottom_ban ul li:first-child').animate({"margin-left":"-"+first_img+"px"},5000 );
	    to=setTimeout('ban_remove()',6000);
	};
$(document).ready(function() {
	var to=0;
	var my_temp="";
	$('.bottom_ban ul').mouseover(function () {
			clearTimeout(to);
			//$('.bottom_ban ul li:first-child').stop();
	});
	$('.bottom_ban ul').mouseout(function () {
			to=setTimeout('ban_remove()',6000);
	});
