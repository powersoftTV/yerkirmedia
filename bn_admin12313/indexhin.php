...
if(!isset($_SERVER['PHP_AUTH_USER']) || !$_SERVER['PHP_AUTH_USER']){
	header ("WWW-Authenticate: Basic realm=\"BizNet Admin Page\"");
 ...
 <?}//ip>0
	...
	<?
	 if($act=="news"){
	  ...
	  if($_GET['id_d']) {
		 mysql_query("DELETE FROM `records` WHERE `id`='".intval($_GET['id_d'])."' ");
		 }
		echo"<p style='text-align:right;height:25px;'><a href='?act=news&n=1'>Ավելացնել նորություն</a></p>";
	  if(!$id && !isset($_GET['n'])){

	   $resh = mysql_query("SELECT * FROM `records` where `r_category` > 0 AND `r_show_hy` = 0 ORDER BY `r_ord` desc ");
	   $count = @mysql_num_rows($resh);

	   echo"<h2 style='height:40px;font-size:18px;'>Չհրապարակված նորությունները` ".$count." հատ</h2>";

	   if(!$count) {
		  echo"Չհրապարակված նորություններ չկան:";
	   }
	   else{
       ...
       for($i=0;$i<$count;$i++)	{
    	        $id_r = mysql_result($resh, $i, "id");
    	        $r_category_r = mysql_result($resh, $i, "r_category");
				$r_title_hy_r = mysql_result($resh, $i, "r_title_hy");
	   ...
	   $resh = mysql_query("SELECT * FROM `records` where `r_category` > 0 AND `r_show_hy` = 1 ORDER BY `r_ord` desc ");
	   $count = @mysql_num_rows($resh);

	   echo"<h2 style='height:40px;font-size:18px;'>Հրապարակված նորություները` ".$count." հատ</h2>";
	   ...
	   echo"<table cellpadding='0' cellspacing='0'><tr style='background: #d4d4d4; height:24px;'><td width='30'></td><td  width='360'><b>Վերնագիր<b></td><td width='20'></td><td width='150'><b>Բաժին</b></td><td width='20'></td><td width='120'><b>Հրապարակվել է</b></td></tr>";
	     if($count) $note_right_text.= "Հրապարակված նորությունները դասավորված են ըստ նրանց ստեղծման, ամենավերջինները ամենասկզբում: Հրապարակված նորությունները նույնպես կարող եք փոփոխել սեղմելով ցանկացած մեկի վերնագրի վրա:";
	     else echo"<tr><td colspan='5'><br>Հրապարակված նորություններ չկան:</td></tr>";
       for($i=($p-1)*10;$i<$count && $i<(10*$p);$i++)	{
    	        $id_r = mysql_result($resh, $i, "id");
    	        $r_category_r = mysql_result($resh, $i, "r_category");
				...
				echo"<tr><td height='30'><span style='color:#aaaaa;'>".$j."</span></td>
				         <td>".$tImage."<a href='?act=news&id=".$id_r."'>".$r_title_hy_r."</a></td>
				         <td width='20'></td>
						 <td><span style='color:#aaaaa;'>".$c_name_hy."</span></td>
						 <td width='20'></td>
	  ...
	  elseif($id || (isset($_GET['n']) && $_GET['n']==1)){
		 ...
		 $note_right_text.= "Նորությունը փոփոխելուց հետո, սեղմում եք ներքևում տեղակայված &quot;Թարմացնել ինֆորմացին&quot; կոճակը, փոփոխված ինֆորմացիան բազայում գրանցելու համար:";
	  ...
	  ?><form action='#' method='post' name='EditForm' enctype='multipart/form-data'>
	    ...
	    <table>
		 ...
		 <tr>
		   <td style='height:40px;' valign='top'> Գլխավոր նկարը(360x280)</td><td style="padding-left:30px;" valign='top'>
